...
int main(int argc, char** argv) {
    ...
    for (u32 frame = 1; frame <= options.frames; ++frame) {
        ...
        std::printf(
            "frame=%u file=%s pc=0x%08X cpsr=0x%08X halted=%d IE=0x%04X IF=0x%04X DISPSTAT=0x%04X IME=%d cycle=%llu rom_reads=%llu rom_misses=%llu rom_hits=%llu rom_seq_hits=%llu rom_unique_pages=%u rom_prefetches=%llu rom_prefetch_misses=%llu rom_prefetch_hits=%llu rom_prefetch_bytes=%llu rom_miss_us=%llu rom_miss_cycles=%llu\n",
            frame,
            output_path.c_str(),
            state.regs[15],
            state.cpsr,
            ...
            static_cast<unsigned long long>(rom_stats.sequential_hits),
            rom_stats.unique_pages,
            static_cast<unsigned long long>(rom_stats.prefetches),
            static_cast<unsigned long long>(rom_stats.prefetch_misses),
            static_cast<unsigned long long>(rom_stats.prefetch_hits),
            static_cast<unsigned long long>(rom_stats.prefetch_bytes),
            static_cast<unsigned long long>(rom_stats.miss_penalty_us),
            static_cast<unsigned long long>(rom_stats.miss_penalty_cycles)
        );
    }
